...
int XDemo(int numSamples, double *allRMSE, double *allRMSE2, int fold_id,
  ...
  HPMF *obj =
      new HPMF(num_columns, num_latent_features, numNodesPerLevel,
               predict_level, num_hierarchy_level, used_num_hierarchy_level,
               gaps, burn, numSamples, fold_id, input_dir, saveFlag,
               outWholeFlag, mean_file_path, std_file_path, verbose);
...
SEXP DemoHPMF(SEXP args, SEXP mean_file_pathSexp, SEXP std_file_pathSexp,
  ...
  for (int i = 0; i < 4; i++) SET_STRING_ELT(resultNames, i, mkChar(names[i]));
