...
void mvrnorm(double *des, double *mu, double *cholCov, int dim, bool upper) {
  ...
  for (i = 0; i < dim; i++) des[i] = rnorm(0.0, 1.0);
...
/*
void dlmreadVec(char *filePath, matVecFloat &myMat, int &nRow, int nCol, int
verbose)

Read a text file into a matrix, a very simple code with assumption number of
columns of matrix is known.
*/
void dlmreadVec(string filePath, matVecFloat &myMat, int &nRow, int nCol,
  ...
  for (int ii = 0; ii < nRow; ii++) {
    ...
    for (int jj = 0; jj < nCol; jj++) myMat[ii][jj] = myArr[ii * nCol + jj];
  }
  if (verbose) {
...
/*
void dlmreadVec(char *filePath, matVecInt &myMat, int &nRow, int nCol, int
verbose)
...
/*
void dlmreadVec(string filePath, matVecInt &myMat, int nRow, vector<int> nCols,
int verbose) {
