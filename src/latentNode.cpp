...
NodeLatent::NodeLatent(int num_parent, int num_child, int par_idx,
    ...
    : num_parent_(num_parent),
      ...
      num_latent_features_(num_feat),
      num_observ_(0) {
  if (num_child) children_ = new NodeLatent *[num_child];
...
NodeLatent::~NodeLatent() { SafeDelete(children_); }
...
void NodeLatent::UpdatePrior(int mode) {
  ...
  for (int ind = 0; ind < num_parent_; ind++) {
    F77_NAME(daxpy)
    (&num_latent_features_, &cc, parents_->GetLatentFactor(), &inc_one,
     prior_parent.get(), &inc_one);
  }
  for (int ind = 0; ind < num_child_; ind++) {
    F77_NAME(daxpy)
    (&num_latent_features_, &cc, children_[ind]->GetLatentFactor(), &inc_one,
  ...
  F77_NAME(daxpy)
  (&num_latent_features_, &cc, prior_parent.get(), &inc_one, prior_.get(),
   &inc_one);
  F77_NAME(daxpy)
  (&num_latent_features_, &cc, prior_child.get(), &inc_one, prior_.get(),
   &inc_one);
}
...
void NodeLatent::UpdateGradient(int freq, double err, double *vv) {
  ...
  F77_NAME(daxpy)
  (&num_latent_features_, &err, vv, &inc_one, gradient_.get(), &inc_one);
}
...
void NodeLatent::UpdateLatentFactor(double coef, double momentum,
  ...
  F77_NAME(daxpy)
  (&num_latent_features_, &cc, prior_.get(), &inc_one, gradient_.get(),
   &inc_one);
  ...
  // deV[j] += coef * v[j]
  F77_NAME(daxpy)
  (&num_latent_features_, &coef, latent_factor_.get(), &inc_one,
   gradient_.get(), &inc_one);

  F77_NAME(dscal)
  (&num_latent_features_, &momentum, old_latent_factor_.get(), &inc_one);
  // old = old * momentum + epsilon * deV[j]
  F77_NAME(daxpy)
  (&num_latent_features_, &epsilon, gradient_.get(), &inc_one,
   old_latent_factor_.get(), &inc_one);

  cc = -1;
  // latent_factor_ -= old;
  F77_NAME(daxpy)
  (&num_latent_features_, &cc, old_latent_factor_.get(), &inc_one,
...
void NodeLatent::UpdatePrior() {
  ...
  for (int ind = 0; ind < num_parent_; ind++) {
    F77_NAME(daxpy)
    (&num_latent_features_, &one, parents_->GetLatentFactor(), &inc_one,
     prior_parent.get(), &inc_one);
  }

  for (int ind = 0; ind < num_child_; ind++) {
    F77_NAME(daxpy)
    (&num_latent_features_, &one, children_[ind]->GetLatentFactor(), &inc_one,
     prior_child.get(), &inc_one);
  }

  F77_NAME(daxpy)
  (&num_latent_features_, &one, prior_parent.get(), &inc_one, prior_.get(),
   &inc_one);

  F77_NAME(daxpy)
  (&num_latent_features_, &one, prior_child.get(), &inc_one, prior_.get(),
   &inc_one);
}
...
void NodeLatent::GibbsUpdate(NodeLatent ***v_tree, double sig_inv,
  ...
  std::unique_ptr<double[]> cov(
      new double[num_latent_features_ * num_latent_features_]);
  std::unique_ptr<double[]> latent_trans(
      new double[num_observ_ * num_latent_features_]);
  ...
  for (int ind = 0; ind < num_observ_; ind++) {
    ...
    copy(tmp, tmp + num_latent_features_,
         latent_trans.get() + ind * num_latent_features_);

    // mu += v_j * r_{ij} / sig
    F77_NAME(daxpy)
    (&num_latent_features_, &beta, tmp, &inc_one, mu.get(), &inc_one);
  }

  // mu += prior / sigU ; prior = sum(u_ch) + u_pr
  F77_NAME(daxpy)
  (&num_latent_features_, &sig_u_inv, prior_.get(), &inc_one, mu.get(),
   &inc_one);
  ...
  F77_NAME(dgemm)
  (ntrans, ytrans, &num_latent_features_, &num_latent_features_, &num_observ_,
   &sig_inv, latent_trans.get(), &num_latent_features_, latent_trans.get(),
   &num_latent_features_, &numPrior, cov.get(), &num_latent_features_);
  ...
  F77_NAME(dgemv)
  (ntrans, &num_latent_features_, &num_latent_features_, &one, cov.get(),
   &num_latent_features_, mu.get(), &inc_one, &zero, mu_u.get(), &inc_one);

  F77_NAME(dpotrf)
  (lower, &num_latent_features_, cov.get(), &num_latent_features_, &info);
