...
/*HPMF::HPMF(const char *input_dir, const int num_columns, int
num_latent_features, int num_hierarchy_level, int used_hierarchy_level, int
predict_level, int fold_id, int saveFileFlag, int outWholeFlag, int gaps, int
burn, int num_samples, const char* mean_file_path, const char* std_file_path,
double* num_nodes_per_level)
*/
HPMF::HPMF(const int num_columns, int num_latent_features,
           double* num_nodes_per_level, int predict_level,
           int num_hierarchy_level, int used_hierarchy_level, int gaps,
           int burn, int num_samples, int fold_id, const char* input_dir,
  ...
  if (used_hierarchy_level >= predict_level) {
    Rprintf(
        "Error: wrong number of parameters: used_hierarchy_level should be "
        "smaller than predict_level\n");
    // error();
  }
  if (verbose_) {
  ...
  this->num_nodes_per_level_.assign(
      num_nodes_per_level, num_nodes_per_level + num_hierarchy_level + 1);
  ...
  *************************************/
  // string test_var = string(input_dir_) + "/fold" + to_string(static_cast<long
  // double>(fold_id_))
...
void HPMF::IntializeHierarchy() {
  int num_rows =
      accumulate(num_nodes_per_level_.begin(), num_nodes_per_level_.end(), 0);
  ...
  if ((used_hierarchy_level_ == predict_level_ - 1) ||
      (used_hierarchy_level_ == num_hierarchy_level_))
    return;

  int parent_level_start_ind =
      accumulate(num_nodes_per_level_.begin(),
  ...
  for (int level = used_hierarchy_level_ + 1; level < predict_level_; level++) {
    ...
    for (int node_ind = start_ind; node_ind < last_ind; node_ind++) {
      ...
      for (int parent_ind = 0; parent_ind < num_parents_[node_ind];
        ...
        new_parents_list.insert(new_parents_list.end(), grant_parent.begin(),
                                grant_parent.end());
      }
  ...
  for (int node_ind = parent_level_start_ind;
       node_ind <
       parent_level_start_ind + num_nodes_per_level_[predict_level_ - 2];
       node_ind++) {
    for (int parent_ind = 0; parent_ind < num_parents_[node_ind];
         parent_ind++) {
      int parent = hierarchy_[node_ind][parent_ind];
  ...
  for (int level = 0; level <= used_hierarchy_level_; level++) {
    ...
    if (level == used_hierarchy_level_) {
      for (int ii = level; ii < predict_level_ - 1; ii++) {
        if (!num_nodes) last_level_ind += num_nodes_per_level_[ii];
      }
    ...
    for (int node = 0; node < num_rows; node++) {
      ...
      if (num_nodes) {  // treeV
        ...
        num_children = (level == used_hierarchy_level_)
                           ? 0
                           : num_children_[node + last_level_ind];
        num_parents = (used_hierarchy_level_ == 0)
                          ? 0
                          : num_parents_[node + last_level_ind];
        parents = (used_hierarchy_level_ == 0)
                      ? empty_parent
                      : hierarchy_[node + last_level_ind];
      }
      ...
      if (!level) {  // if it is at the root of tree with no parent
        ...
        tree[level][node] =
            new NodeLatent(num_parents, num_children, parents[0], tree,
                           num_latent_features_, level, parLevel);
      }
    }
...
void HPMF::SetObservation() {
  for (int level = 0; level <= used_hierarchy_level_; level++) {
    ...
    *************************************/
    int tmp_level = lev + 1;
    // string train_var = string(input_dir_) + "/fold" +
    // to_string(static_cast<long double>(fold_id_))
...
double HPMF::Predict(int u_ind, int v_ind, int level) {
  ...
  double pred =
      F77_NAME(ddot)(&num_latent_features_, uu, &inc_one, vv, &inc_one);
  return pred;
}
